...
battery_icon.forced_width = dpi(20)
...
watch('acpi -i', 10, function(_, stdout)
    ...
    for s in stdout:gmatch('[^\r\n]+') do
        local bat, status, charge_str, time = string.match(s, '(.+): .*')

        if available[bat] == nil then
            available[bat] = 1
        else
            available[bat] = available[bat] + 1
        end
    end
    local battery_info = {}
    local capacities = {}
    for s in stdout:gmatch('[^\r\n]+') do
        local bat = string.match(s, '(.+): .*')
        local status, charge_str, time = string.match(s, '.+: (%a+), (%d?%d?%d)%%,?.*')

        if available[bat] > 1 then
            if status ~= nil then
